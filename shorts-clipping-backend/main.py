...
app = modal.App("ai-shorts-clipping", image=image)

volume = modal.Volume.from_name(
    "ai-shorts-clipping-cache", create_if_missing=True
)
...
def create_vertical_video(
    ...
    for fidx, fname in tqdm(enumerate(flist), total=len(flist), desc="Creating vertical video"):
        ...
        if mode == "resize":
            ...
            img_cropped = resized_img[0:target_height,
...
def process_clip(
    ...
    # vertical video path - vertical video from start to end
    vertical_mp4_path = clip_dir / "pyavi" / "video_out_vertical.mp4"

    # vertical video with subtitles
    subtitle_output_path = clip_dir / "pyavi" / "video_out_vertical_sub.mp4"
    ...
    # cmds
    cut_cmd = (f"ffmpeg -i {orginal_video_path} -ss {start_time} -t {duration} "
               f" {clip_segment_path}")
    ...
    print(
        f"Columbia processing took {columbia_end_time - columbia_start_time:.2f} seconds")

    tracks_path = clip_dir / "pywork" / "tracks.pckl"
    scores_path = clip_dir / "pywork" / "scores.pckl"
    if not tracks_path.exists() or not scores_path.exists():
        raise FileNotFoundError(
            f"Tracks or scores file not found in {clip_dir / 'pywork'}")
    ...
    print(
        f"Clip {clip_index} vertical video creation took {cvv_end_time - cvv_start_time:.2f} seconds")
    ...
    s3_client.upload_file(
        subtitle_output_path, "ai-shorts-clipping", output_s3_key)
...
@app.cls(gpu="T4", timeout=900, retries=0, scaledown_window=20, volumes={mount_path: volume}, secrets=[modal.Secret.from_name("ai-shorts-clipping-secret")])
class AIShortsClipping:
    @modal.enter()
    def load_modal(self):
        ...
        self.whisperx_align_model, self.metadata = whisperx.load_align_model(
            language_code="en",
            device="cuda",
        )
    ...
    def transcribe_video(
        ...
        print(f"Transcription and alingment took {duration:.2f} seconds")
    ...
    def process_video(
        self,
        request: ProcessVideoRequest, token:
        ...
        # dowmload the video from S3
        video_path = base_dir / "input.mp4"
        s3_client = boto3.client('s3')
        s3_client.download_file("ai-shorts-clipping", s3_key, str(video_path))
        ...
        if base_dir.exists():
            print(f"Cleaning up base directory... {base_dir}")
...
def main():
    ...
    ai_shorts_clipping = AIShortsClipping()

    url = ai_shorts_clipping.process_video.web_url

    payload = {
        "s3_key": "test2/joe-short-longer.mp4"
    }
